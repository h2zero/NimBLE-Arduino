...
#ifdef ESP_PLATFORM
#include "esp_err.h"
#define NIMBLE_CORE (MYNEWT_VAL(NIMBLE_PINNED_TO_CORE) < portNUM_PROCESSORS ? MYNEWT_VAL(NIMBLE_PINNED_TO_CORE) : tskNO_AFFINITY)
#define NIMBLE_HS_STACK_SIZE (MYNEWT_VAL(NIMBLE_HOST_TASK_STACK_SIZE))
#else
#define NIMBLE_HS_STACK_SIZE (MYNEWT_VAL(NIMBLE_HOST_TASK_STACK_SIZE) / 4)
#endif
